...
export function ActivityForm({ today }: { today: string }) {
  ...
  const [state, action] = useActionState(
    async (previous: ActionState, formData: FormData) => {
      const result = await createActivityAction(previous, formData);
      if (result.status === "success")
        setFields({ workDate: today, hours: "", description: "" });
      return result;
    },
    initialActionState,
  );
            ...
            <div className="grid gap-5 sm:grid-cols-2">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setFields({ ...fields, workDate: e.target.value })
                  }
                  required
                />
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  value={fields.hours}
                  onChange={(e) =>
                    setFields({ ...fields, hours: e.target.value })
                  }
                  placeholder="e.g. 4.5"
                  required
                />
